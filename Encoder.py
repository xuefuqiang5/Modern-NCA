...
class PLREncoder(nn.Module): 
    def __init__(self, d_in, n_freq, d_embedding, freq_init_scale=0.1): 
        super().__init__()
...
class FeatureEncoder(nn.Module): 
    def __init__(self, n_num_features, cat_cardinalitics, d_num_embedding, d_cat_embedding, plr_freq=48, is_embedding=False):
        super().__init__()
    ...
    def forward(self, x_cat, x_num): 
        if self.is_embedding: 
            x_offset = (x_cat + self.offset).long()         
            x_cat = self.embedding(x_offset)
            x_cat = rearrange(x_cat, "... n d ->... (n d)")
        else: 
            
            indices = (self.offset + x_cat).long()
